...
Repository.prototype.createCommitOnHead = function(
  ...
  repo.openIndex()
    .then(function(index_) {
      ...
      filesToAdd.forEach(index.addByPath);
      index.write();
 ...
 * @param {Remote.AUTOTAG_OPTION|autoTag} set the auto tag option for remote
 ...
 * @param {Object|RemoteCallback} remoteCallbacks Any custom callbacks needed
 * @param {Remote.AUTOTAG_OPTION|autoTag} set the AUTO_TAG option for remotes
 * @param {Bool|pruneAfter} will perform a prune after the fetch if true
 */

 ...
 * @param {Function}   beforeNextFn    Callback to be called before each invocation of next(). If the callback returns a
 *                                     promise, the next() will be called when the promise resolves.
 * @param {Function}   beforeFinishFn  Callback called before the invocation of finish(). If the callback returns a
 *                                     promise, finish() will be called when the promise resolves. This callback will be
 *                                     provided a detailed overview of the rebase.
 * @returns {number | Index} An error code for an unsuccesful rebase or an index for a rebase with conflicts.
 */
 ...
 * @param {number} acrossFs     If non-zero, then the lookup will not stop when a filesystem device change is detected
 *                              while exploring parent directories.
 * @param {string} ceilingDirs  A list of absolute symbolic link free paths.
 *                              the search will stop if any of these paths are hit. This may be set to null.
...
/**
 * This will set the HEAD to point to the local branch and then attempt to update the index and working tree to match
 * the content of the latest commit on that branch.
 *
...
/**
 * This will set the HEAD to point to the reference and then attempt to update the index and working tree to match the
 * content of the latest commit on that reference.
 *
...
Repository.prototype.checkoutRef = function (reference, opts) {
  ...
  opts.checkoutStrategy =
    opts.checkoutStrategy || NodeGit.Checkout.STRATEGY.SAFE | NodeGit.Checkout.STRATEGY.RECREATE_MISSING;
  return repo
 ...
 *
 * @param {Signature}     signature       Identity of the one performing the rebase.
 * @param {Function}      beforeNextFn    Callback to be called before each step of the rebase. If the callback returns
 *                                        a promise, the rebase will resume when the promise resolves. The rebase object
 *                                        is is passed to the callback.
 * @param {Function}      beforeFinishFn  Callback called before the invocation of finish(). If the callback returns a
 *                                        promise, finish() will be called when the promise resolves. This callback will
 *                                        be provided a detailed overview of the rebase.
 ...
 * @async
 */
Repository.prototype.createCommit = function (updateRef, author, committer, message, tree, parents) {
  var repo = this;
  var promises = [];
 ...
 * @param {Array}        parents
 * @returns {Promise<string>} The content of the commit object as a string.
 * @async
 */
  ...
  const createCommitPromise = Promise.all(promises)
    ...
    .then(function ({ code, field, signedData }) {
      switch (code) {
        ...
        default: {
          const error = new Error("Repository.prototype.createCommitWithSignature " + `threw with error code ${code}`);
          error.errno = code;
          throw error;
        }
      }
 ...
 *
 * @param {string | Oid} oid string sha or Oid.
 ...
 *
 * @param {string} name short or full tag name.
 * @async
 */
...
Repository.prototype.discardLines = function (filePath, selectedLines, additionalDiffOptions) {
  ...
  return repo
    ...
    .then(function (newContent) {
      return FilterList.load(repo, null, filePath, Filter.MODE.SMUDGE, Filter.FLAG.DEFAULT).then(function (
        _filterList
      ) {
        filterList = _filterList;
        if (filterList) {
 ...
 * @param {Object | FetchOptions} fetchOptions  Options for the fetch, includes callbacks for fetching.
 * @async
 */
...
/**
 * Fetches from all remotes. This is done in series due to deadlocking issues with fetching from many remotes that can
 * happen.
 *
 * @param {Object | FetchOptions} fetchOptions  Options for the fetch, includes callbacks for fetching.
 * @async
...
/**
 * Get extended statuses of a repo to it's working directory. Status entries have `status`, `headToIndex` delta, and
 * `indexToWorkdir` deltas.
 *
 * @param {obj} opts
 * @returns {Promise<StatusFile>}
 * @async
...
Repository.prototype.mergeBranches = function (to, from, signature, mergePreference, mergeOptions, mergeBranchOptions) {
  ...
  if (isOldOptionParameter) {
    console.error(
      "DeprecationWarning: Repository#mergeBranches parameter " + "processMergeMessageCallback, use MergeBranchOptions"
    );
  }
  const processMergeMessageCallback =
    (mergeBranchOptions &&
      (isOldOptionParameter ? mergeBranchOptions : mergeBranchOptions.processMergeMessageCallback)) ||
    function (message) {
      return message;
    };
 ...
 * @param {boolean} isSelectionStaged       Are the selected lines currently staged.
 * @returns {number} 0 or an error code.
 * @async
 */

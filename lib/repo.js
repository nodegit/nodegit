...
exports.repo = function(directory, callback) {
  ...
  if (directory) {
    ...
    fs.exists(directory, function directoryExists(exists) {
      if (!exists) {
        callback(new Error('Directory must exist'), null);
        return;
      }
      fs.realpath(directory, function realpathCallback(error, directory) {
        if (error) {
  ...
  self.commit = function(sha, callback) {
    git.commit().lookup(self.repo, sha, function(error, commit) {

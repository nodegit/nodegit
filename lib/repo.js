...
Repo.prototype.branch = function(name, callback) {
   ...
   */
  var self = this;
  this.rawRepo.getReference('refs/heads/' + name, function referenceLookupCallback(error, reference) {
    if (error) return callback(error);
...
Repo.prototype.commit = function(oid, callback) {
  ...
  try {
    ...
    this.rawRepo.getCommit(oid, function(error, rawCommit) {
      if (error) return callback(error);
...
Repo.prototype.blob = function(oid, callback) {
   ...
   */
  this.rawRepo.getBlob(oid.rawOid, function blobLookup(error, rawBlob) {
    if (error) return callback(error);
...
Repo.prototype.tree = function(oid, callback) {
   ...
   */
  var self = this;
  this.rawRepo.getTree(oid.rawOid, function(error, rawTree) {
    if (error) return callback(error);

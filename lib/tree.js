...
Tree.prototype.diff = function(that, callback) {
  this.rawTree.diffTree(this.repo.rawRepo, that.rawTree, null, function(error, rawDiffList) {
    if (error) return callback(error);

    callback(null, new git.diffList(rawDiffList));
  });
};

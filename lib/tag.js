 ...
 * @param {number}     force
 * @param {Function}   signingCallback  Takes a string and returns a string representing the signed message.
 * @returns {Oid}
 * @async
 */
Tag.createWithSignature = async (repo, tagName, target, tagger, message, force, signingCallback) => {
  let targetOid;
  if (!target.id) {
  ...
  switch (code) {
    ...
    case NodeGit.Error.CODE.PASSTHROUGH:
      return Tag.create(repo, tagName, targetOid, tagger, message, force);
    default: {

    ...
    it("returns non-binary filtered content when checking binary", function () {
      ...
      return commitFile(test.repository, attrFileName, filter, "added gitattributes")
        ...
        .then(function (lfBlob) {
          ...
          return test.lfBlob.filter(newFileName, { flags: NodeGit.Blob.FILTER_FLAG.CHECK_FOR_BINARY });
        })
        .then(function (content) {
          var ending = content.match(lineEndingRegex);
          assert.strictEqual(ending[0], "\r\n");
    ...
    it("returns nothing when checking binary blob", function () {
      ...
      return commitFile(test.repository, attrFileName, filter, "added gitattributes")
        ...
        .then(function (binaryBlob) {
          ...
          return test.binaryBlob.filter(newFileName, { flags: NodeGit.Blob.FILTER_FLAG.CHECK_FOR_BINARY });
        })
        .then(function (content) {
          assert.strictEqual(content, "");
        });

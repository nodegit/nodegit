...
describe("Cherrypick", function () {
  ...
  it("can cherrypick a commit onto another specified commit", function () {
    ...
    return RepoUtils.setupBranches(repo)
      ...
      .then(function () {
        assert(fse.existsSync(path.join(workDirPath, repoInfo.ourFileName)), repoInfo.ourFileName + " should exist");
        assert(
          fse.existsSync(path.join(workDirPath, repoInfo.theirFileName)),
          repoInfo.theirFileName + " should exist"
        );
  ...
  it("can cherrypick a stash to apply it", function () {
    ...
    return RepoUtils.setupBranches(repo, true)
      ...
      .then(function (statuses) {
        ...
        return fse.writeFile(path.join(workDirPath, repoInfo.ourFileName), repoInfo.ourFileContent + addedContent);
      })
      .then(function () {
        return repo.getStatus();
      })

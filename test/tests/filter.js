  ...
  beforeEach(function () {
    ...
    return Repository.open(reposPath)
      ...
      .then(function (emptyRepo) {
        ...
        return fse.writeFile(path.join(reposPath, ".gitattributes"), "*.md filter=" + filterName + " -text", {
          encoding: "utf-8",
        });
      });
  });
  ...
  describe("Apply", function () {
    ...
    it("should not apply when check returns GIT_PASSTHROUGH", function () {
      ...
      return Registry.register(
        ...
        .then(function (result) {
          assert.strictEqual(result, NodeGit.Error.CODE.OK);
          return fse.writeFile(packageJsonPath, "Changing content to trigger checkout", { encoding: "utf-8" });
        })
        .then(function () {
          var opts = {
    ...
    it("can run sync callback on checkout without deadlocking", function () {
      // jshint ignore:line
      var test = this;
    ...
    it.skip("can run async callback on checkout without deadlocking", function () {
      // jshint ignore:line
      var test = this;

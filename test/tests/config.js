    ...
    it("opens the config and can change a value", function () {
      ...
      return Repository.open(reposPath)
        ...
        .then(function (_repoConfig) {
          ...
          return Promise.all([onDiskConfig.getString("core.filemode"), repoConfig.getString("core.filemode")]);
        })
        .then(function (results) {
          var onDiskFileMode = results[0];
        ...
        .then(function (config) {
          return Promise.all([config.getString("core.filemode"), repoConfig.getString("core.filemode")]);
        })
        .then(function (results) {
          var onDiskFileMode = results[0];
          var repoFileMode = results[1];

...
describe("Stage", function () {
  ...
  it("can stage/unstage filemode changes for multiple files", function () {
    ...
    return Promise.all(
      ...
      .then(function () {
        //change the permission on index
        return exec("git update-index --chmod=+x " + fileName[0], { cwd: test.repository.workdir() });
      })
      .then(function () {
        //change the permission on index
        return exec("git update-index --chmod=+x " + fileName[1], { cwd: test.repository.workdir() });
      })
      .then(function () {
        //Commit the change with execute bit set

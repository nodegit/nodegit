...
describe("Merge", function () {
    ...
    return (
      fse
        ...
        .then(function (oid) {
          ...
          return repository.createCommit(ourBranch.name(), ourSignature, ourSignature, "we merged their commit", oid, [
            ourCommit,
            theirCommit,
          ]);
        })
  ...
  it("can merge 2 branchs with conflicts on a single file", function () {
    ...
    return fse
      ...
      .then(function (oid) {
        ...
        return repository.createCommit(ourBranch.name(), ourSignature, ourSignature, "lol big bobs :yesway:", oid, [
          baseCommit,
        ]);
      })
      .then(function (commitOid) {
      ...
      .then(function (oid) {
        ...
        return repository.createCommit(theirBranch.name(), theirSignature, theirSignature, "lol big bobs :poop:", oid, [
          baseCommit,
        ]);
      })
      .then(function (commitOid) {
  ...
  it("leaves repo in MERGE state after a standard merge with conflicts fails", function () {
    ...
    return fse
      ...
      .then(function (oid) {
        return Promise.all([repository.getBranchCommit(ourBranchName), repository.getBranchCommit("MERGE_HEAD")]).then(
          function (commits) {

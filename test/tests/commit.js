...
describe("Commit", function () {
  ...
  describe("amendWithSignature", function () {
    ...
    it("will respects overridden arguments", function () {
      ...
      var committer = NodeGit.Signature.create("Shaggy Rogers", "shaggy@mystery.com", 987654321, 90);
      ...
      return NodeGit.Repository.open(reposPath)
        ...
        .then(function (headCommit) {
          ...
          return headCommit.amendWithSignature(null, author, committer, null, null, tree, onSignature);
        })
        .then(function (oidResult) {
          oid = oidResult;
          return NodeGit.Commit.lookup(repo, oid);
  ...
  describe("Commit's Body", function () {
    it("null if only summary", function () {
      var test = this;
      return NodeGit.Commit.lookup(test.repository, "15315cf41ad76400d9189c85a5827b77b8c392f1").then(function (commit) {
    ...
    it("non-null when body exists", function () {
      var test = this;
      return NodeGit.Commit.lookup(test.repository, "c82fb078a192ea221c9f1093c64321c60d64aa0d").then(function (commit) {
    ...
    it("Can create a signed commit in a repo", function () {
      ...
      return NodeGit.Repository.open(reposPath)
        ...
        .then(function (signatures) {
          ...
          return repo.createCommitWithSignature(null, author, committer, "message", treeOid, [parent], onSignature);
        })
        .then(function (commitId) {
          assert.equal(expectedCommitId, commitId);
    ...
    it("Can create a signed commit in a repo and update existing ref", function () {
      ...
      return NodeGit.Repository.open(reposPath)
        ...
        .then(function (signatures) {
          ...
          return repo.createCommitWithSignature("HEAD", author, committer, "message", treeOid, [parent], onSignature);
        })
        .then(function (commitId) {
          assert.equal(expectedCommitId, commitId);
          return NodeGit.Commit.lookup(repo, commitId);
    ...
    it("Can create a signed commit in bare a repo and update non-existent ref", function () {
      ...
      return NodeGit.Repository.init(newRepoPath, 0)
        ...
        .then(function (signatures) {
          ...
          return repo.createCommitWithSignature("HEAD", author, committer, "message", treeOid, [], onSignature);
        })
        .then(function (commitId) {
          assert.equal(expectedCommitId, commitId);
          return NodeGit.Commit.lookup(repo, commitId);

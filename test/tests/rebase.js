...
describe("Rebase", function () {
  ...
  it("can cleanly fast-forward via rebase", function () {
    ...
    return (
      fse
        ...
        .then(function () {
          return Promise.all([repository.getReference(ourBranchName), repository.getReference(theirBranchName)]);
        })
        .then(function (refs) {
        ...
        .then(function (annotatedCommits) {
          ...
          return NodeGit.Rebase.init(repository, ourAnnotatedCommit, theirAnnotatedCommit, theirAnnotatedCommit);
        })
    ...
    return (
      fse
        ...
        .then(function (oid) {
          ...
          return repository.createCommit(ourBranch.name(), ourSignature, ourSignature, "we made a commit", oid, [
            ourCommit,
          ]);
        })
        .then(function (commitOid) {
        ...
        .then(function (annotatedCommits) {
          ...
          return NodeGit.Rebase.init(repository, ourAnnotatedCommit, theirAnnotatedCommit, undefined, rebaseOptions);
        })
  ...
  it("can rebase 2 branches with conflicts on a single file", function () {
    ...
    return fse
      ...
      .then(function (oid) {
        ...
        return repository.createCommit(theirBranch.name(), theirSignature, theirSignature, "they made a commit", oid, [
          ourCommit,
        ]);
      })
      .then(function (commitOid) {
        ...
        return fse.writeFile(path.join(repository.workdir(), fileName), baseFileContent + ourFileContent);
      })
      .then(function () {
        return RepoUtils.addFileToIndex(repository, fileName);
      })
      ...
      .then(function () {
        return fse.readFile(path.join(repository.workdir(), fileName), "utf8").then(function (fileContent) {
  ...
  it("can abort an in-progress rebase", function () {
    ...
    return (
      fse
        ...
        .then(function () {
          return Promise.all([repository.getBranchCommit("HEAD"), repository.getBranchCommit(ourBranchName)]);
        })
  ...
  it(
    "can fast-forward a merge commit via rebase using the " + "convenience methods that has a beforeFinishFn",
    function () {
      var ourFileName = "ourNewFile.txt";
      var theirFileName = "theirNewFile.txt";
      ...
      var ourSignature = NodeGit.Signature.create("Ron Paul", "RonPaul@TollRoadsRBest.info", 123456789, 60);
      var theirSignature = NodeGit.Signature.create("Greg Abbott", "Gregggg@IllTollYourFace.us", 123456789, 60);
      var theirOtherSignature = NodeGit.Signature.create("Greg Abbott", "Gregggg@IllTollYourFace.us", 123456999, 60);
      var ourMergeSignature = NodeGit.Signature.create("Ron Paul", "RonPaul@TollRoadsRBest.info", 123456889, 60);
      ...
      return (
        fse
          ...
          .then(function () {
            return repository.checkoutBranch(ourBranch, { checkoutStrategy: Checkout.STRATEGY.FORCE });
          })
          .then(function () {
          ...
          .then(function () {
            return repository.checkoutBranch(theirBranch, { checkoutStrategy: Checkout.STRATEGY.FORCE });
          })
          .then(function () {
            return fse.writeFile(path.join(repository.workdir(), theirOtherFileName), theirOtherFileContent);
          })
          .then(function () {
          ...
          .then(function (annotatedCommits) {
            ...
            assert.equal(ourAnnotatedCommit.id().toString(), "0d1d322b59df68bac6eea6a2a189f974cb590368");
            assert.equal(theirAnnotatedCommit.id().toString(), "8fa0ce25a2accf464b004ddeeb63add7b816b627");
  ...
  it("can rebase using the convenience method", function () {
    ...
    return (
      fse
        ...
        .then(function () {
          return repository.rebaseBranches(ourBranchName, theirBranchName, null, ourSignature, function (rebase) {
    ...
    it("can sign commits during the rebase", function () {
      ...
      return (
        fse
          ...
          .then(function (commitOid) {
            assert.equal(commitOid.toString(), "617cd03370dd799f372e9dcfcd0b097aede1bd7f");
          ...
          .then(function (commit) {
            // verify that the "ours" branch has moved to the correct place
            assert.equal(commit.id().toString(), "617cd03370dd799f372e9dcfcd0b097aede1bd7f");

            return Promise.all([
              commit.parent(0),
              NodeGit.Commit.extractSignature(repository, "617cd03370dd799f372e9dcfcd0b097aede1bd7f", "moose-sig"),
            ]);
          })
    ...
    return (
      fse
        ...
        .then(function (oid) {
          ...
          return repository
            .createCommit(theirBranch.name(), theirSignature, theirSignature, "they made a commit", oid, [
              initialCommit,
            ])
            .then(function (commitOid) {
        ...
        .then(function (annotatedCommits) {
          ...
          assert.equal(ourAnnotatedCommit.id().toString(), "5814ffa17b8a677191d89d5372f1e46d50d976ae");
          assert.equal(theirAnnotatedCommit.id().toString(), "c4cc225184b9c9682cb48294358d9d65f8ec42c7");
  ...
  it("rebase signs correctly if rebaseOptions are re-used", function () {
    ...
    return (
      fse
        ...
        .then((oid) => {
          assert.equal(oid.toString(), "76631cb5a290dafe2959152626bb90f2a6d8ec94");
          return repository.createCommit("HEAD", theirSignature, theirSignature, "they made a commit", oid, [
            ourCommit,
          ]);
        })
        .then((commitOid) => {
        ...
        .then((_theirCommit) => {
          ...
          return Promise.all([
            NodeGit.AnnotatedCommit.lookup(repository, ourCommit.id()),
            NodeGit.AnnotatedCommit.lookup(repository, theirCommit.id()),
          ]);
        })
        ...
        .then((_newCommitOid) => {
          ...
          assert.strictEqual(newCommitOid.toString(), "9909e435b52322a71dc341d747b29c392a34c745");
          return rebase.next();
        })
        .then(() => {
        ...
        .catch((error) => {
          ...
          return NodeGit.Commit.extractSignature(repository, newCommitOid.toString(), "moose-sig");
        })
        .then((sig) => {
